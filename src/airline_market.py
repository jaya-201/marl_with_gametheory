...
class AirlineMarketEnv(ParallelEnv):
    ...
    def __init__(self, airlines=AGENTS):
        super().__init__()
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            predicted_pax = max(0, predicted_pax + np.random.normal(0, 2))
            passengers_sold[agent] = predicted_pax
            rewards[agent] = prices[self.agent_to_idx[agent]] * predicted_pax

        self.days_left -= 1
        terminations = {agent: self.days_left <= 0 for agent in self.agents}
        truncations = {agent: False for agent in self.agents}
        observations = {agent: self._get_obs(agent) for agent in self.agents}
        infos = {agent: {"passengers": passengers_sold[agent]} for agent in self.agents}
